...
发布脚本会展示上一个 Tag 以来的提交记录，并询问目标版本和发布说明。发布说明支持单行或多行输入：每次按回车会继续输入下一行，单独输入一行 `.` 并回车才会结束；单行说明输入完成后，也需要再输入一行 `.`。如果不准备手写说明，直接输入 `.`，脚本会使用上面展示的提交列表。

最终确认后，脚本会同步并校验全部版本文件，创建 `chore: 发布 vX.Y.Z` 版本提交和带说明的 annotated Tag，再原子推送当前分支与 Tag。`chore: 发布 vX.Y.Z` 只用于记录版本号变更，不会作为更新说明展示。工作区不干净、版本不一致或 Tag 已存在时会停止发布。

工作流会从 GitHub 远端 annotated Tag 原样读取发布说明，将其同时写入 GitHub Release 正文和 `latest.json`，应用更新弹窗最终显示的就是这段内容。如果 Tag 不是 annotated Tag 或说明为空，发布会直接失败，不会退回到版本提交信息。原子推送失败时，远端分支和 Tag 都不会更新，本地版本提交与 Tag 会保留，可按脚本提示修复后重试。

只需推送 Tag，不要提前在 GitHub 页面手动创建或发布 Release。推送 Tag 后，工作流会在 Windows x64、macOS Intel、macOS Apple Silicon 全部构建完成后创建 Release；如果同 Tag 的 Release 已存在，则会复用、覆盖为 Tag 中的发布说明并补齐产物。尚未集成 Updater 的旧版本需要手动安装一次新版，此后才能使用应用内更新。
