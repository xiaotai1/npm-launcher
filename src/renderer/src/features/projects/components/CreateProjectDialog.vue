...
<style scoped>
.create-dialog-backdrop { position: fixed; inset: 0; z-index: 1100; display: grid; place-items: center; padding: 28px; background: rgba(4, 9, 17, .52); backdrop-filter: blur(3px); }
.create-dialog { width: min(460px, calc(100vw - 40px)); overflow: hidden; border: 1px solid var(--border-strong); border-radius: 14px; color: var(--text-primary); background: var(--bg-surface); box-shadow: var(--shadow-lg); }
...
.dialog-form { padding: 18px 20px 0; }.field-group { display: flex; flex-direction: column; gap: 7px; margin-bottom: 16px; }.field-group label { color: var(--text-secondary); font-size: 11px; font-weight: 650; }.field-group input,.field-group select { width: 100%; min-height: 40px; font-size: 12px; }.field-group input[readonly] { color: var(--text-secondary); cursor: pointer; }.path-control { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 8px; }.browse-button { min-width: 68px; border: 1px solid var(--accent-border); border-radius: 8px; color: var(--accent-primary); background: var(--bg-surface); font-size: 11px; font-weight: 700; }.browse-button:hover:not(:disabled) { background: var(--accent-glow); }.field-hint { min-height: 16px; color: var(--text-secondary); font-size: 10px; }
